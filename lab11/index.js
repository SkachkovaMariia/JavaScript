    ...
    function showGallery() {
        ...
        fetch('/galleryImages')
            ...
            .then(images => {
                ...
                galleryDiv.style.gridTemplateColumns = 'repeat(auto-fill, minmax(150px, 1fr))';
            ...
            .catch(error => {
                ...
                content.textContent = 'Failed to fetch gallery images';
            })
            .finally(() => {
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    leftPanel.appendChild(weatherBlock);

    function updateWeather() {
        fetch('/weather')
            .then(response => response.json())
            .then(weatherData => {
                const { city, temperature } = weatherData;
                weatherBlock.textContent = `City: ${city}, Temperature: ${temperature}°C`;
            })
            .catch(error => {
                console.error('Error fetching weather:', error);
                weatherBlock.textContent = 'Failed to fetch weather';
            });
    }
